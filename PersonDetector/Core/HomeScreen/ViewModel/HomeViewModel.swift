...
protocol HomeViewModelProtocol {
    ...
    func saveImageToDB(image: UIImage, date: Date, isPersonDetected: Bool)
}
...
final class HomeViewModel: HomeViewModelProtocol {
    ...
    func detectPerson(with image: UIImage) {
        ...
        Task(priority: .userInitiated) {
            do {
                let result = try await personDetector.detectPerson(with: image) // 0.45 sn
                ...
                await MainActor.run {
                    ...
                    print(" Son Detect Süre: \(duration) saniye") // 0.50 - 0.57 sn 160 - 230 mb arası ram kullanımı
                }
                ...
                if error as! PersonDetectorError == PersonDetectorError.detectionFailed {
                    ...
                    self.saveImageToDB(image: image, date: Date(), isPersonDetected: false)
                    let endTime = Date()
                    let duration = endTime.timeIntervalSince(startTime)
                    print(" No Person Detect Süre: \(duration) saniye") //0.45 - 0.48 sn
                }
            }
            let endTime = Date()
            let duration = endTime.timeIntervalSince(startTime)
            print(" Son Süre: \(duration) saniye")
        }
